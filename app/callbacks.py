...
class StreamingCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_start(self, *args, **kwargs) -> None:
        ...
        self.queue.put_nowait(json.dumps({"event": "start", "id": self.current, "parent" : str(run_parent) if run_parent else None, "tags": run_tags}))
        self.current += 1

    def on_llm_new_token(self, token: str, **kwargs) -> None:
        run_parent = kwargs.get("parent_run_id")
        run_tags = kwargs.get("tags", [])
        self.queue.put_nowait(json.dumps({"event": "new_token", "id": self.current, "data": token, "parent" : str(run_parent) if run_parent else None, "tags": run_tags}))
        self.current += 1

    def on_llm_end(self, *args, **kwargs) -> None:
        run_parent = kwargs.get("parent_run_id")
        run_tags = kwargs.get("tags", [])
        self.queue.put_nowait(json.dumps({"event": "llm_end", "id": self.current, "parent" : str(run_parent) if run_parent else None, "tags": run_tags}))
        self.current += 1
    ...
    def on_tool_start(self, tool_input: str, tool_name: str, **kwargs) -> None:
        self.queue.put_nowait(json.dumps({"event": "tool_start", "id": self.current, "data": {"tool_name": tool_name, "tool_input": tool_input}}))
        self.current += 1
...
class BasicCallbackHandler(BaseCallbackHandler):
    ...
    def on_agent_finish(
        ...
        **kwargs: Any
        ...
        messages = [
            MessageOutput(content="".join(buf))
